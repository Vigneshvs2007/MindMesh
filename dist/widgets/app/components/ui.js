...
export function PhaseBadge({ phase, size = 'md', showLabel = true }) {
    ...
    return (<span style={{
      ...
      {showLabel && phaseInfo && (<span style={{ opacity: 0.9 }}>{phaseInfo.name}</span>)}
      <span style={{ fontFamily: DESIGN.fonts.mono }}>P{phase}</span>
    </span>);
}
export function ToolChip({ name, phase, active = false, onClick, showPhase = true, size = 'sm' }) {
    ...
    return (<button onClick={onClick} disabled={!onClick} style={{
            display: 'inline-flex',
            alignItems: 'center',
            ...
            if (!active && onClick)
                e.currentTarget.style.color = DESIGN.colors.fgMuted;
        }}>
      <code style={{ fontSize: 11 }}>{name}</code>
      {showPhase && phase !== undefined && (<span style={{
                fontSize: 9,
                padding: '1px 4px',
...
export function StatusDot({ status, size = 8, label }) {
    const statusConfig = {
...
export function StatusIndicator({ status, text, className = '', style }) {
    return (<span className={className} style={{ display: 'inline-flex', alignItems: 'center', gap: DESIGN.spacing.xs, ...style }}>
...
export function Card({ children, elevated = false, padding = 'md', className = '', onClick, style }) {
...
export function Input({ label, icon, error, className = '', style = {}, inputStyle, ...props }) {
    return (<div className={className} style={{ display: 'flex', flexDirection: 'column', gap: DESIGN.spacing.xs, ...style }}>
      {label && (<label style={{ fontSize: 11, fontWeight: 600, color: DESIGN.colors.fgMuted, textTransform: 'uppercase', letterSpacing: 0.5, fontFamily: DESIGN.fonts.mono }}>
          {label}
        </label>)}
      <div style={{ display: 'flex', alignItems: 'center', position: 'relative' }}>
        {icon && (<span style={{ position: 'absolute', left: DESIGN.spacing.sm, color: DESIGN.colors.fgDim, pointerEvents: 'none', fontSize: 14 }}>{icon}</span>)}
        <input {...props} style={{
            width: '100%',
...
export function Button({ variant = 'primary', size = 'md', loading = false, leftIcon, rightIcon, disabled, children, className = '', style = {}, onClick, ...props }) {
    const variants = {
        primary: {
    ...
    return (<button {...props} onClick={onClick} disabled={isDisabled} className={className} style={{
            display: 'inline-flex',
            alignItems: 'center',
