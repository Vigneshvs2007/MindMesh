...
import { PhaseBadge, Input, Button, Badge } from '../components/ui';
const ALL_TOOLS = [
    // Phase 0
...
export default function PhaseSearchBar() {
    ...
    const currentPhase = 3; // In production: from widget data
    const phaseName = 'Synthesis & Clustering'; // In production: from widget data
    ...
    }, [state.showSuggestions, state.selectedIndex, state.showTools]);
    const getSuggestions = useCallback(() => {
        const allSuggestions = [];
        // Phase-specific tool suggestions
        const phaseTools = ALL_TOOLS.filter(t => t.phase === currentPhase);
        ...
        if (state.query.trim()) {
            ...
            return allSuggestions.filter(s => s.text.toLowerCase().includes(q) ||
    ...
    const executeSearch = () => {
        ...
        // In production: useWidgetSDK().postMessage({ type: 'search', data: { query: state.query, phase: currentPhase } });
        console.log('Search:', state.query, 'Phase:', currentPhase);
    };
    const executeTool = (toolName) => {
        // In production: useWidgetSDK().postMessage({ type: 'tool', data: { tool: toolName, phase: currentPhase } });
        console.log('Execute tool:', toolName, 'Phase:', currentPhase);
        addToHistory(toolName, currentPhase);
    };
    const addToHistory = (query, phase) => {
        const newItem = {
            query,
            phase,
    ...
    const handlePhaseToolClick = (tool) => {
    ...
    return (<div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: DESIGN.spacing.sm }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: DESIGN.spacing.xs }}>
            ...
            <Button variant="ghost" size="sm" onClick={() => setState(prev => ({ ...prev, showTools: !prev.showTools }))} style={{ padding: '4px 8px' }}>
      ...
      <div style={{
        ...
        <div style={{ position: 'relative' }}>
          <Input ref={inputRef} placeholder={`Search Phase ${currentPhase}... (type tool name or query)`} value={state.query} onChange={handleInputChange} onFocus={handleFocus} onBlur={handleBlur} onKeyDown={handleKeyDown} icon={<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>} style={{ width: '100%' }}/>
          {state.query && (<Button variant="ghost" size="sm" onClick={executeSearch} style={{
                position: 'absolute',
        ...
        {state.query && (<div style={{ marginTop: DESIGN.spacing.xs, display: 'flex', gap: DESIGN.spacing.xs, flexWrap: 'wrap' }}>
      ...
      {(state.showSuggestions || state.showTools) && (<div ref={suggestionsRef} style={{
                position: 'relative',
          ...
          {/* Tools Panel */}
          {state.showTools && currentPhaseTools.length > 0 && (<div style={{ padding: DESIGN.spacing.md }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: DESIGN.spacing.sm }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: DESIGN.spacing.xs }}>
                {currentPhaseTools.map(tool => (<div key={tool.name} style={{
                        display: 'flex',
                    ...
                    }} onClick={() => handlePhaseToolClick(tool)} onMouseEnter={(e) => { e.currentTarget.style.borderColor = phaseColor; }} onMouseLeave={(e) => { e.currentTarget.style.borderColor = DESIGN.colors.border; }}>
          ...
          {/* Suggestions List */}
          {!state.showTools && suggestions.length > 0 && (<div style={{ padding: DESIGN.spacing.xs }}>
              {suggestions.map((suggestion, index) => (<div key={`${suggestion.type}-${suggestion.text}-${index}`} style={{
                        display: 'flex',
                        alignItems: 'center',
                        gap: DESIGN.spacing.sm,
                        padding: `${DESIGN.spacing.sm}px ${DESIGN.spacing.md}px`,
                        ...
                        cursor: 'pointer',
                        transition: DESIGN.transitions.fast,
                    }} onClick={() => selectSuggestion(suggestion)} onMouseEnter={() => setState(prev => ({ ...prev, selectedIndex: index }))}>
                  <div style={{
                    ...
                    {suggestion.type === 'tool' && (<svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/></svg>)}
                    {suggestion.type === 'query' && (<svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>)}
                    {suggestion.type === 'history' && (<svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>)}
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <code style={{ fontSize: 12, color: DESIGN.colors.fg, fontFamily: DESIGN.fonts.mono }}>
                      {suggestion.text}
                    </code>
                    {suggestion.tool && (<span style={{ fontSize: 9, color: DESIGN.colors.fgDim, fontFamily: DESIGN.fonts.mono, marginLeft: DESIGN.spacing.xs }}>
                        Tool: {suggestion.tool}
                      </span>)}
                  </div>
                  <Badge variant={suggestion.type === 'tool' ? 'info' : suggestion.type === 'history' ? 'success' : 'default'} size="sm">
          ...
          {/* Empty State */}
          {!state.showTools && suggestions.length === 0 && state.query && (<div style={{ padding: DESIGN.spacing.lg, textAlign: 'center', color: DESIGN.colors.fgDim }}>
          ...
          {/* History Section */}
          {!state.showTools && state.history.length > 0 && !state.query && (<div style={{ borderTop: `1px solid ${DESIGN.colors.border}`, padding: DESIGN.spacing.md }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: DESIGN.spacing.sm }}>
                <span style={{ fontSize: 11, fontWeight: 600, color: DESIGN.colors.fgMuted, textTransform: 'uppercase', letterSpacing: 0.5, fontFamily: DESIGN.fonts.mono }}>
                  Recent Searches
                </span>
                {state.history.length > 5 && (<Button variant="ghost" size="sm" onClick={clearHistory} style={{ fontSize: 10 }}>
                    Clear
                  </Button>)}
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: DESIGN.spacing.xs }}>
                {state.history.slice(0, 8).map((item, index) => (<div key={index} style={{
                    ...
                    }} onMouseEnter={(e) => { e.currentTarget.style.borderColor = DESIGN.colors.borderBright; }} onMouseLeave={(e) => { e.currentTarget.style.borderColor = DESIGN.colors.border; }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: DESIGN.spacing.xs, flexWrap: 'wrap' }}>
          <Button variant="ghost" size="sm" onClick={() => setState(prev => ({ ...prev, showTools: true, showSuggestions: false }))} style={{ padding: '6px 10px', fontSize: 10 }}>
          ...
          {PHASE_GROUPS.flatMap(g => g.phases).map(p => (<Button key={p.id} variant={currentPhase === p.id ? 'primary' : 'ghost'} size="sm" onClick={() => {
                // In production: navigate to phase
                console.log('Navigate to phase', p.id);
            }} style={{
