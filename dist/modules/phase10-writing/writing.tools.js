 ...
 */
let WritingTools = class WritingTools {
    memory;
    constructor(memory) {
        this.memory = memory;
    }
    async checkWriting(input, ctx) {
    ...
    checkMeaningPreserved(section, text, originalText) {
        if (!originalText) {
            return {
                checkId: generateId('write'),
                section,
    ...
    extractKeyClaims(text) {
        ...
        return sentences.filter(s => /we (find|show|demonstrate|prove|observe|achieve|improve|outperform|propose|introduce|develop)/i.test(s) ||
...
], WritingTools.prototype, "checkWriting", null);
WritingTools = __decorate([
    Injectable({ deps: [MemoryStore] }),
    __metadata("design:paramtypes", [MemoryStore])
], WritingTools);
export { WritingTools };
//# sourceMappingURL=writing.tools.js.map