 ...
 */
let AnalogyTools = class AnalogyTools {
    memory;
    embeddings;
    semanticScholar;
    ...
    async findCrossDomainAnalogs(input, ctx) {
        ...
        const scored = await Promise.all(uniquePapers.map(async (paper) => {
    ...
    buildAnalogQueries(technique, targetDomain, excludeDomains = []) {
        const baseQueries = [
    ...
    inferDomain(paper) {
        ...
        if (venue.includes('cvpr') || venue.includes('iccv') || fields.includes('Computer Vision'))
            return 'Computer Vision';
        if (venue.includes('nips') || venue.includes('icml') || venue.includes('iclr') || fields.includes('Machine Learning'))
            return 'Machine Learning';
        if (venue.includes('acl') || venue.includes('emnlp') || fields.includes('NLP'))
            return 'NLP';
        if (venue.includes('siggraph') || fields.includes('Graphics'))
            return 'Graphics';
        if (venue.includes('robotics') || fields.includes('Robotics'))
            return 'Robotics';
        if (venue.includes('bio') || fields.includes('Biology'))
            return 'Biology';
        if (venue.includes('physics') || fields.includes('Physics'))
            return 'Physics';
    ...
    assessTransferability(similarity) {
        if (similarity > 0.7)
            return 'high';
        if (similarity > 0.5)
            return 'medium';
    ...
    async verifyTechniqueMatch(input, ctx) {
...
], AnalogyTools.prototype, "findCrossDomainAnalogs", null);
__decorate([
    Tool({
    ...
    __metadata("design:paramtypes", [Object, Object]),
    __metadata("design:returntype", Promise)
], AnalogyTools.prototype, "verifyTechniqueMatch", null);
AnalogyTools = __decorate([
...
export { AnalogyTools };
//# sourceMappingURL=analogy.tools.js.map